  ...
  submitbtnsign = (e) => {
    ...
    if (name == null) {
      ...
      const date =
        today.getFullYear() +
        '-' +
        monthsandday[today.getMonth() + 1] +
        '-' +
        monthsandday[today.getDate()]
      const time =
        today.getHours() + ':' + today.getMinutes() + ':' + today.getSeconds()
      const dateTime = date + ' ' + time
      axios
        .post(`${process.env.REACT_APP_SERVER}/user/siginup`, {
          password: password,
        })
        .then((res) => {
          ...
          if (res.data.status !== false) {
            firebase
              .auth()
              .createUserWithEmailAndPassword(email, res.data.msg)
              .then((userCredential) => {
                // Signed in
    ...
    return (
      <div>
        <div className='card'>
          ...
          {!submitbtnsign ? (
            <button className='btn btn-primary' onClick={this.submitbtnsign}>
              Sign Up
            </button>
          ) : (
          ...
          <p className='subtitle'>
            <a href={'/register'}> Become A Doctor</a>
          </p>
        </div>
        <ToastContainer />
      </div>
